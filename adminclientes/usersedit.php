...
class cusers_edit extends cusers {
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->usu_id->FldIsDetailKey)
			$this->usu_id->setFormValue($objForm->GetValue("x_usu_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->usu_id->CurrentValue = $this->usu_id->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// usu_id
			$this->usu_id->EditCustomAttributes = "";
			$this->usu_id->EditValue = $this->usu_id->CurrentValue;
			$this->usu_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		$sFilter = $this->KeyFilter();
			...
			$sFilterChk = "(`usu_login` = '" . ew_AdjustSql($this->usu_login->CurrentValue) . "')";
		...
		if ($rs->EOF) {
			...
			// usu_nombre
			$this->usu_nombre->SetDbValueDef($rsnew, $this->usu_nombre->CurrentValue, "", $this->usu_nombre->ReadOnly);

			// usu_login
			$this->usu_login->SetDbValueDef($rsnew, $this->usu_login->CurrentValue, "", $this->usu_login->ReadOnly);

			// usu_pass
			$this->usu_pass->SetDbValueDef($rsnew, $this->usu_pass->CurrentValue, "", $this->usu_pass->ReadOnly);

			// emp_id
			$this->emp_id->SetDbValueDef($rsnew, $this->emp_id->CurrentValue, NULL, $this->emp_id->ReadOnly);

			// per_id
			$this->per_id->SetDbValueDef($rsnew, $this->per_id->CurrentValue, NULL, $this->per_id->ReadOnly);

			// usu_vigente
			$this->usu_vigente->SetDbValueDef($rsnew, $this->usu_vigente->CurrentValue, 0, $this->usu_vigente->ReadOnly);
...
fusersedit.Lists["x_emp_id"] = {"LinkField":"x_emp_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_emp_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fusersedit.Lists["x_per_id"] = {"LinkField":"x_per_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_per_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $users->TableCaption() ?></span></p>
...
<table id="tbl_usersedit" class="ewTable">
...
<?php echo $users->usu_id->EditValue ?></span>
<input type="hidden" name="x_usu_id" id="x_usu_id" value="<?php echo ew_HtmlEncode($users->usu_id->CurrentValue) ?>">
</span><?php echo $users->usu_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<input type="password" name="x_usu_pass" id="x_usu_pass" value="<?php echo $users->usu_pass->EditValue ?>" size="30" maxlength="16"<?php echo $users->usu_pass->EditAttributes() ?>>
...
<script type="text/javascript">
fusersedit.Lists["x_emp_id"].Options = <?php echo (is_array($users->emp_id->EditValue)) ? ew_ArrayToJson($users->emp_id->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
fusersedit.Lists["x_per_id"].Options = <?php echo (is_array($users->per_id->EditValue)) ? ew_ArrayToJson($users->per_id->EditValue, 1) : "[]" ?>;
</script>
