...
$informes_archivo_blobview = NULL; // Initialize page object first

class cinformes_archivo_blobview extends cinformes {
	...
	// Page object name
	var $PageObjName = 'informes_archivo_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('informes', 'x_archivo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('archivo');
				...
				if (trim(strval($this->Recordset->fields('archivo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('archivo') . "\"");
				}
...
// Create page object
if (!isset($informes_archivo_blobview)) $informes_archivo_blobview = new cinformes_archivo_blobview();

// Page init
$informes_archivo_blobview->Page_Init();

// Page main
$informes_archivo_blobview->Page_Main();
?>
<?php
$informes_archivo_blobview->Page_Terminate();
?>
