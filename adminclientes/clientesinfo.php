...
class cclientes extends cTable {
	...
	function __construct() {
		...
		// logo
		$this->logo = new cField('clientes', 'clientes', 'x_logo', 'logo', '`logo`', '`logo`', 201, -1, FALSE, '`logo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['logo'] = &$this->logo;

		// mailsalerta
		$this->mailsalerta = new cField('clientes', 'clientes', 'x_mailsalerta', 'mailsalerta', '`mailsalerta`', '`mailsalerta`', 200, -1, FALSE, '`mailsalerta`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['mailsalerta'] = &$this->mailsalerta;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("clientesview.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->mailsalerta->Exportable) $Doc->ExportCaption($this->mailsalerta);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->nombre->Exportable) $Doc->ExportCaption($this->nombre);
				if ($this->mailsalerta->Exportable) $Doc->ExportCaption($this->mailsalerta);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->mailsalerta->Exportable) $Doc->ExportField($this->mailsalerta);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->nombre->Exportable) $Doc->ExportField($this->nombre);
					if ($this->mailsalerta->Exportable) $Doc->ExportField($this->mailsalerta);
