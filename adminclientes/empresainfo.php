...
class cempresa extends cTable {
	...
	function __construct() {
		...
		// emp_id
		$this->emp_id = new cField('empresa', 'empresa', 'x_emp_id', 'emp_id', '`emp_id`', '`emp_id`', 3, -1, FALSE, '`emp_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->emp_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['emp_id'] = &$this->emp_id;

		// emp_nombre
		$this->emp_nombre = new cField('empresa', 'empresa', 'x_emp_nombre', 'emp_nombre', '`emp_nombre`', '`emp_nombre`', 200, -1, FALSE, '`emp_nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['emp_nombre'] = &$this->emp_nombre;
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('emp_id') . '=' . ew_QuotedValue($rs['emp_id'], $this->emp_id->FldDataType) . ' AND ';
		}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("empresaview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "empresaadd.php";
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("empresadelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->emp_nombre->Exportable) $Doc->ExportCaption($this->emp_nombre);
			} else {
				if ($this->emp_id->Exportable) $Doc->ExportCaption($this->emp_id);
				if ($this->emp_nombre->Exportable) $Doc->ExportCaption($this->emp_nombre);
			}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->emp_nombre->Exportable) $Doc->ExportField($this->emp_nombre);
				} else {
					if ($this->emp_id->Exportable) $Doc->ExportField($this->emp_id);
					if ($this->emp_nombre->Exportable) $Doc->ExportField($this->emp_nombre);
				}
