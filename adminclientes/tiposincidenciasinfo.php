...
//
// Table class for tiposincidencias
//
class ctiposincidencias extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'tiposincidencias';
		$this->TableName = 'tiposincidencias';
		...
		// tipi_id
		$this->tipi_id = new cField('tiposincidencias', 'tiposincidencias', 'x_tipi_id', 'tipi_id', '`tipi_id`', '`tipi_id`', 3, -1, FALSE, '`tipi_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tipi_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['tipi_id'] = &$this->tipi_id;
	...
	// Update Table
	var $UpdateTable = "`tiposincidencias`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('tipi_id') . '=' . ew_QuotedValue($rs['tipi_id'], $this->tipi_id->FldDataType) . ' AND ';
		}
	...
	function SqlKeyFilter() {
		return "`tipi_id` = @tipi_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->tipi_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@tipi_id@", ew_AdjustSql($this->tipi_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "tiposincidenciaslist.php";
		}
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("tiposincidenciasview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "tiposincidenciasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("tiposincidenciasedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("tiposincidenciasadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tiposincidenciasdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->tipi_id->CurrentValue)) {
			$sUrl .= "tipi_id=" . urlencode($this->tipi_id->CurrentValue);
		} else {
