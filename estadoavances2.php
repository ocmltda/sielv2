...
else
{
	...
	if ($db2->nf() > 0)
	{
		while($db2->next_record())
		{
			...
			$db->query('SELECT DAY(V.fecha_visita) AS DIA, Count(V.fecha_visita) AS TOTDIA FROM visitas AS V WHERE year(V.fecha_visita) = ' . $_REQUEST['anio'] . ' and month(V.fecha_visita)= ' . $_REQUEST['mes'] . ' GROUP BY V.fecha_visita ORDER BY V.fecha_visita ASC');
			if ($db->nf() > 0)
			{
				$tf = 1;
				while($db->next_record())
			...
			$db->query('SELECT V.estado_visita AS ESTADO, Count(V.estado_visita) AS TOTESTADO FROM visitas AS V WHERE year(V.fecha_visita) = ' . $_REQUEST['anio'] . ' and month(V.fecha_visita)= ' . $_REQUEST['mes'] . ' GROUP BY V.estado_visita ORDER BY ESTADO ASC');
			if ($db->nf() > 0)
			{
				$tf = 1;
				while($db->next_record())
			...
			$db->query('SELECT Count(V.fecha_visita) AS TOTPRG FROM visitas AS V WHERE year(V.fecha_visita) = ' . $_REQUEST['anio'] . ' and month(V.fecha_visita)= ' . $_REQUEST['mes'] . ' AND (V.estado_visita = 0 OR V.estado_visita = 1)');
			$db->next_record();
			$t->assign("_ROOT.tep", $db->Record['TOTPRG'] . '');

			//total evaluaciones realizadas
			$db->query('SELECT Count(V.fecha_visita) AS TOTREA FROM visitas AS V WHERE year(V.fecha_visita) = ' . $_REQUEST['anio'] . ' and month(V.fecha_visita)= ' . $_REQUEST['mes'] . ' AND V.estado_visita = 1');
			$db->next_record();
			$t->assign("_ROOT.ter", $db->Record['TOTREA'] . '');

			//total evaluaciones en curso
			$db->query('SELECT Count(V.fecha_visita) AS TOTENC FROM visitas AS V WHERE year(V.fecha_visita) = ' . $_REQUEST['anio'] . ' and month(V.fecha_visita)= ' . $_REQUEST['mes'] . ' AND V.estado_visita = 0');
			$db->next_record();
